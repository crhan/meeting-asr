...
def _styled_diff_text(diff_text: str) -> Text:
    ...
    while index < len(lines):
        ...
        if _is_removed_diff_line(line):
            ...
            _append_changed_line_block(rendered, removed, added)
            continue
        if _is_added_diff_line(line):
            added, index = _collect_diff_lines(lines, index, _is_added_diff_line)
            _append_changed_line_block(rendered, [], added)
            continue
...
def _append_changed_line_block(rendered: Text, removed: list[str], added: list[str]) -> None:
    ...
    for index in range(count):
        ...
        if old_line is not None and new_line is not None:
            old_segments, new_segments = _word_diff_segments(old_line[1:], new_line[1:])
            _append_segmented_line(rendered, "-", old_segments, removed=True)
            _append_segmented_line(rendered, "+", new_segments, removed=False)
            continue
...
def _word_diff_segments(old_text: str, new_text: str) -> tuple[list[tuple[str, bool]], list[tuple[str, bool]]]:
