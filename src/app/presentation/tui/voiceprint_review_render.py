...
def project_sample_summary(sample: VoiceprintCaptureClipEntry | None) -> str:
    ...
    return f"{project_sample_time_range(sample)} | capture {sample.selection_score:.3f} | {sample.selection_reason} | {state}"
...
def project_sample_line(sample: VoiceprintCaptureClipEntry) -> str:
    ...
    return f"{project_sample_time_range(sample)} {tag} capture={sample.selection_score:.3f} {trim_text(sample.text)}"
