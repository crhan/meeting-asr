...
def render_help(command: Any, command_path: tuple[str, ...]) -> None:
    """
    Render localized help for a Typer command with Rich panels.

    Args:
        command: Typer command (``typer.core.TyperGroup``/``TyperCommand``).
    ...
    ctx = typer.Context(command, info_name=" ".join(("meeting-asr", *command_path)))
...
def _usage_text(command: Any, ctx: Any, lang: str) -> Text:
...
def _help_body(command: Any, command_path: tuple[str, ...], lang: str) -> Text:
...
def _print_arguments_panel(command: Any, lang: str, console: Console) -> None:
    """Print positional arguments when a command has them."""
    rows = [param for param in command.params if isinstance(param, TyperArgument)]
    if not rows:
        return
    table = _table()
...
def _print_options_panel(
    command: Any, command_path: tuple[str, ...], lang: str, console: Console
) -> None:
    ...
    if not any("--help" in option.opts for option in options):
        rows.append(_implicit_help_row(lang))
    if not rows:
        return
    table = _table()
    table.add_column(style="cyan", no_wrap=True)
    ...
    for names, metavar, help_text in rows:
        table.add_row(names, metavar, help_text)
...
def _implicit_help_row(lang: str) -> tuple[str, str, str]:
    """Build the implicit help row for commands that rely on Typer defaults."""
    help_text = OPTION_ZH["--help"] if lang == "zh" else "Show this message and exit."
    return ("--help, -h", "", help_text)


def _option_names(option: Any) -> str:
...
def _option_metavar(option: Any) -> str:
...
def _option_help(option: Any, command_path: tuple[str, ...], lang: str) -> str:
...
def _command_description(command: Any, command_path: tuple[str, ...], lang: str) -> str:
...
def _argument_required_text(argument: Any, lang: str) -> str:
