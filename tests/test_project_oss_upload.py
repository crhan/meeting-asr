...
def test_project_oss_upload_reuses_existing_project_object_without_upload(
    ...
    def fail_upload_file_to_oss(*args, **kwargs) -> str:
        raise AssertionError("upload_file_to_oss should not be called for a reusable object")
...
def test_project_oss_upload_uploads_when_manifest_has_no_object_key(
    ...
    def fail_presign_oss_object(*args, **kwargs) -> str:
        raise AssertionError("presign_oss_object should not be called without object_key")

    monkeypatch.setattr(project_manager, "presign_oss_object", fail_presign_oss_object)
    monkeypatch.setattr(project_manager, "estimate_oss_upload", lambda settings, *, size_bytes: None)
    monkeypatch.setattr(project_manager, "upload_file_to_oss", fake_upload_file_to_oss)
    monkeypatch.setattr(project_manager, "record_oss_upload", lambda settings, **kwargs: None)
    ...
    assert source == "oss_signed_url"
    assert uploaded[0]["kwargs"]["object_name"] == f"meeting-asr/projects/{manifest.project_id}/audio.flac"
...
def test_project_oss_upload_uploads_when_manifest_object_key_mismatches_audio(
    ...
    def fail_presign_oss_object(*args, **kwargs) -> str:
        raise AssertionError("presign_oss_object should not be called for mismatched object_key")
...
def test_project_oss_upload_falls_back_to_upload_when_presign_fails(
    ...
    assert events[0].description.startswith("Existing OSS object presign failed; uploading audio instead")
