...
- **库体检报出「这两个人现在就会被互认」**:库里两个人的声纹太像时,现有两条检查都看不见——`voiceprint quality` 是簇内一致性,拿样本跟**自己那个人**的质心比,两个会被互认的人各自都可以是满分(他们恰恰是因为彼此一致才危险);`voiceprint health` 此前只查可用性,一个样本充足、向量齐全、跟另一个人分不开的人在体检里是纯绿色。最近邻分数其实早就算出来并展示在声纹页上了,但**没有任何一处拿它跟阈值比、给结论**——一个裸的 0.78 不告诉用户这算不算危险,等于没报。现在已有样本越过接受线报 critical(文案区分「部分样本」与「全部样本」,后者意味着这份声纹跟对方根本分不开),还没越线但差距很小报 warning,动作是补采样本。判据用**当前生效的** `voiceprint.match_threshold` 而不是内置默认值:实测本机库配的是 0.548,拿 0.75 去判会把三对正在互认的人全报成正常。与 `threshold-too-low` 有意并存而不是二选一——同一份证据的两种读法,指向两种不同的修法:调阈值是全局取舍(少误纳就多误拒),修被点名的这两个人的样本则不影响其他任何人。方向不对称也是刻意的:A 有一条样本飘到 B 的质心上而 B 的样本离 A 都很远时只报 A,对称的质心距离会把 B 也拖下水、让人去重采根本没问题的音频。
- **`voiceprint calibrate --json` 增加 `neighbors`**:impostor 扫描本来就把每条样本打到其他每个人的质心上、然后只留最大值并**把是谁扔掉**。现在把这个身份留下来,按人给出最危险的那个对手、最高分、已越线的样本 public id 与样本总数。不额外读一次 embedding,也不重算质心。
