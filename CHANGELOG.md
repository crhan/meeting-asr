...
- **簇数已达到 `--speaker-count` 时跳过 resplit**:它自述是「救 under-split」,但此前无条件跑——请求了 6 人、也出了 6 簇,仍把每个 track 的每句非低信息句都嵌一遍(173 分钟 / 2,912 句的项目上是 2 小时 16 分),而这种情形下按定义没有 under-split 可救,产出只可能是误拆。现在 `project run` / `project speakers rerun` 在转写的 track 数 ≥ manifest 里记录的 `speaker_count_hint` 时跳过这一阶段并说明原因;`project speakers resplit` 预览会提示「run 会在这里跳过」,`--apply` 是用户明确要求的,不受此闸门约束。没传 `--speaker-count` 的项目行为不变。
...
- **库体检报出「别人比自己更像自己」**:一个人的样本被另一个库内条目判赢时,现有两条检查都看不见——`voiceprint quality` 是簇内一致性,拿样本跟**自己那个人**的质心比,一个簇松散的人和一个正好坐在他中间的紧凑的人,两边的一致性都可以很好看;`voiceprint health` 此前只查可用性,一个样本充足、向量齐全、却正在被认成别人的人在体检里是纯绿色。最近邻分数其实早就算出来并展示在声纹页上了,但**没有任何一处拿它跟阈值比、给结论**——一个裸的 0.78 不告诉用户这算不算危险,等于没报。**判据是「谁排第一」而不是「跟别人有多像」**:匹配是把候选排序再对第一名套接受规则,所以一条样本判成乙得 0.82、判成自己得 0.95 仍然每次都挂对名字,按绝对分数报警只会催用户去重采完全没问题的音频。因此这条检查不自己另写一套近似,而是把每条样本送进**生产的判定链路**重放一遍:同一套 project-aware 打分(取整人质心与各稳定项目质心中的最大值)、同一套候选顺序、同一套排序、同一套接受规则(包括「没到阈值但甩开第二名足够多」这条强边距接受)。质心也按生产的口径在**原始向量**上取平均再归一化——嵌入是按模型原样存的、范数本来就不一样(实测参考库里最大最小差 1.48 倍),先把每条样本缩成单位长度会悄悄改变加权。被测样本从它本人的候选数据里剔除,相当于当成没见过的探针。同分时的先后也照抄生产(候选顺序来自 `ORDER BY speakers.name`),并在文案里说明「这一条是同分、由名字序分出的先后」——同一个人被录成两个条目时得到的正是这种同分。别人赢了且被接受报 critical(文案区分「部分样本」与「全部样本」),别人赢了但没被接受、或者自己只赢一点点报 warning,动作都是补采样本。判定用**当前生效的** `voiceprint.match_threshold`:同一份证据在 0.75 的库里只是警告,在 0.60 的库里就是此刻正在挂错的名字。与 `threshold-too-low` 有意并存——后者是纯阈值模型、不模拟排序也不看项目质心,按设计是误纳下界,给库级总量与调阈值这个全局取舍,两者数字对不上是正常的。方向不对称也是刻意的:A 有一条样本被 B 判赢、而 B 的样本都还是自己赢时只报 A,对称的质心距离会把 B 也拖下水。只有**真正排第一**的那个候选才会被点名——第三个人分数更高时,把「也压过 A」的 B 报成会被挂上的名字是错的。
- **`voiceprint calibrate --json` 增加 `neighbors`**:impostor 扫描本来就把每条样本打到其他每个人的质心上、然后只留最大值并**把是谁扔掉**。现在把这个身份留下来,按人给出最危险的那个对手、最高的异人分数、最薄的领先幅度(`min_lead`,负数表示对方已经排在前面)、已经会被判错的样本 public id 与样本总数。不额外读一次 embedding,也不重算质心。
...
- **`web` 默认端口 8765 改为 8766**:本机 unifi-api 服务长期占用 127.0.0.1:8765,每次都要手动 `--port` 绕开。同时同步了 vite dev proxy target 与文档示例端口。**这是用户可见的默认值变更**——若此前依赖 8765,请显式传 `--port 8765`。
- **`.python-version` 从 3.14.3 松绑到 3.14**:uv 的 python-build-standalone 没有 cpython-3.14.3 这个构建,导致 `uv run` 在解释器解析阶段就失败(No interpreter found)、web/CLI 全起不来。`requires-python` 本就只要求 `>=3.14`。

## [0.20.0] - 2026-07-26

### 新增

- **新增声纹库可用性体检 `meeting-asr voiceprint health`（web 上是独立的「库质量」页）**：回答的是「这个人到底能不能被匹配到」，与既有的簇内一致性检查（`voiceprint quality`）互补。一致性检查对两种静默失活结构性失明——样本没有当前 model 的向量（切 provider 后没重跑 `voiceprint embed`），以及样本全被隔离——这两种情况在一致性报告里都渲染成绿色，而人根本不会被匹配到。体检按人给出可用/脆弱/不可用判定与阻塞原因，每条都挂可执行动作。
- **新增匹配阈值校准与代价评估**：扫描库内 genuine/impostor 分数分布，给出建议阈值（取两个分布之间「空档中点」而不是 EER——EER 可能紧贴 impostor 上界、零裕度），并按当前库实时定价任一阈值的误拒/误受条数。web 上可拖动游标本地实时预览，不需要每拖一格打一次后端。
- **新增匹配阈值配置键 `voiceprint.match_threshold`**：所有入口统一走 `resolve_match_threshold()`，包括重指派后的 rematch——否则会表现为「`project run` 时生效、紧接着 stabilization 一跑又变回去」。阈值与 strong-margin 救援（0.65）、crosstalk 串场标记（0.5）耦合，越过这两条线会让相应规则变成死代码或语义重叠，写入前显式展示这些副作用。
- **新增补采样本推荐 `meeting-asr voiceprint sources`（web 上是「补采样本」面板）**：体检能说「某人只有 14s 音频 / 只有一场录音」，但答不了「去哪采」。补采扫描全部项目，按缺口加权排序给出可采的具体项目、说话人与片段，并可就地勾选采集。缺时长的人加权供给、单一来源的人加权新场次；已入库样本的时间区间从供给中扣除，不重复推荐同一句。
- **库体检查出「在别人说话时录的」样本**：另一个人在样本前后半秒内说话时，存进库的参考音频是两个人的混合体。这类样本一致性检查完全发现不了（它们彼此自洽），唯一能看出来的地方是来源项目的转写。判定下沉到单条样本，声纹库页可直接筛出是哪几条并就地补采替换。
- **库体检报告来源项目已删除的样本**：clip 存在声纹库里、不随项目删除，所以这些样本仍正常参与匹配；失去的是复核能力——听不到上下文，重叠检查也跑不了，于是它们是「没查过」而被计为干净。某人全部匹配样本都是这种时升为警告。

### 变更

- **采样片段不再把旁边那个人的声音一起采进来**：clip 窗口的 padding 改为 gap-aware，遇到别人说话就停。实测一通两人电话里，0.5s 的固定 padding 让每条 clip 含 10~13% 的另一个人的声音；把 3 条这样的样本并入后，两人质心 cos 从 0.225 升到 0.323。高风险片段永不预选为默认推荐，但仍然列出且可选——薄来源上，一个知情选择的次品样本胜过没有样本。
- **`meeting-asr web` 绑定 `0.0.0.0` 时打印真实可访问地址**，不再输出无法访问的 `http://0.0.0.0:PORT/`。LAN 地址在前、回环兜底，纯标准库枚举。

### 修复

- **修复库迁移后嵌入被静默跳过**：`clip_path` 是入库时那个 store 的绝对路径，换 `XDG_DATA_HOME` 或迁数据卷后会失效，而 clip 文件其实好端端在当前 store 下。播放/删除早已做了路径 rebase，嵌入漏掉，于是那些人永久缺席匹配池，而 `voiceprint embed` 照样报成功（`Embedded: 0`）。实测本机库 6 个样本因此哑掉。
- **修复声纹样本的来源项目链接在项目已删除时跳到错误页**：改为灰色「来源已删除」标记，不再提供必然失败的链接。项目 id 的解析同时改为扫 manifest 按 id 匹配（`--project-dir` 建的项目目录名与 id 不同），并与 speaker review 页自身的加载条件对齐（corrected 转写优先、需有带 speaker 的非空句），避免把能打开的项目说成已删除。
