...
- **库体检报出「别人比自己更像自己」**:一个人的样本被另一个库内条目判赢时,现有两条检查都看不见——`voiceprint quality` 是簇内一致性,拿样本跟**自己那个人**的质心比,一个簇松散的人和一个正好坐在他中间的紧凑的人,两边的一致性都可以很好看;`voiceprint health` 此前只查可用性,一个样本充足、向量齐全、却正在被认成别人的人在体检里是纯绿色。最近邻分数其实早就算出来并展示在声纹页上了,但**没有任何一处拿它跟阈值比、给结论**——一个裸的 0.78 不告诉用户这算不算危险,等于没报。**判据是「谁排第一」而不是「跟别人有多像」**:匹配是把候选排序再对第一名套阈值,所以一条样本判成乙得 0.82、判成自己得 0.95 仍然每次都挂对名字,按绝对分数报警只会催用户去重采完全没问题的音频。因此比的是竞争——每条样本对别人质心的分数,跟它对**自己的留一质心**(和 genuine 分布同一口径,相当于当成没见过的探针)的分数相比。别人赢了且越过接受线报 critical(文案区分「部分样本」与「全部样本」),别人赢了但没到接受线、或者自己只赢一点点报 warning,动作都是补采样本。判定用**当前生效的** `voiceprint.match_threshold`:同一份证据在 0.75 的库里只是警告,在 0.60 的库里就是此刻正在挂错的名字。与 `threshold-too-low` 有意并存——后者是纯阈值模型、不模拟排序,给库级总量与调阈值这个全局取舍,两者数字对不上是正常的。方向不对称也是刻意的:A 有一条样本被 B 判赢、而 B 的样本都还是自己赢时只报 A,对称的质心距离会把 B 也拖下水。
