...
- **血泪坑（验证时差点踩）**：`apply_project_sentence_reassignments` → `_invalidate_overlapping_voiceprint_samples` 会按 `project_id` + 原 speaker + 时间重叠**删全局声纹库的样本**。project_id 是内容寻址的，所以**拷贝项目做端到端验证时，若 store_dir 指向全局库，会误删真实库样本**。验证务必把 voiceprints.sqlite 也拷一份、`store_dir` 指向拷贝（匹配只读库 BLOB，不需要 clips/normalized 目录）。
- 预览/手动应用：`meeting-asr project speakers resplit <proj>` 默认 dry-run（打印 promotions / unknown 桶 / near-miss + 证据分数，零写盘），`--apply` 调 `apply_project_resplit` 直接修一个已处理完的项目（不必重跑 `project run`）。
