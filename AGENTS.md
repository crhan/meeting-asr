...
- Local voiceprint embedding uses `local-campp` (3D-Speaker CAM++ zh-cn, vendored in `src/app/infra/campplus.py`) as the default provider; `local-speechbrain` remains selectable via the `voiceprint.provider` config key and vectors of both models coexist per model key. SpeechBrain, torch, and torchaudio are standard dependencies, not a `local-voiceprint` extra; do not suggest `uv sync --extra local-voiceprint`.
- Do not add ModelScope SDK or speakerlab as dependencies — neither supports this project's Python floor; the vendored torch-only definition plus a sha256-pinned checkpoint download is deliberate. Match entry points backfill missing library vectors for the resolved model key (`ensure_library_embeddings`), so a default-provider switch must never leave matching silently candidate-less; keep that call when touching `_match_context`.
- `torchaudio.load` requires the optional torchcodec package since torchaudio 2.9 and fails at runtime here. Decode audio with `soundfile` (installed via SpeechBrain) and use torchaudio only for DSP (resample/fbank); SpeechBrain's own `load_audio` already does the equivalent internally.
...
## Project Layout Notes（`tmp/` 的唯一判据：能不能免费重算）

- **项目目录里只有两类派生物，判据是「删了要不要再花钱/再花人工」，不是「像不像临时文件」。** 能用 ffmpeg 从 source 重切回来的（探针 wav、聚类切片、`web_clips`、`project_review_playback`、`speaker_apply_preview`）放 `tmp/`；花钱从 embedding 模型买来的向量、LLM 生成的校正提案、人手工编辑过的 review 文件，一律**不准**放 `tmp/`。唯一真源是 `src/app/project_layout.py`，**新加任何项目内路径都先去那里加常量**，别在业务模块里拼字符串。
- **这条规则的价值就是「`rm -rf <项目>/tmp` 永远安全」。** 0.21.0 之前两类混在一起：`tmp/voiceprint_clips/clip_embeddings.json`（实测 32 个项目合计 145M 付费向量）、`tmp/voiceprint_match/probe_embeddings.json`、以及整个 `tmp/corrections/`。结果就是这个目录谁也不敢删、只会一直涨（实测涨到 8.9G wav + 171M json）。现在向量在 `embeddings/`，校正产物在 `corrections/`。
- **`clip_embeddings.json` 缺失 ≠ 会重新 embedding，wav 缺失才是真·免费。** `speaker_cluster_quality._embedded_clips` 只在 `cache.get(key) is None` 时调 `embed_audio_file()`；wav 不在就只是重新 `_write_clip()` 切一刀。所以删 wav 零成本，删 json 是真金白银。改这块前先看清这个分界。
...
- **血泪坑（验证时差点踩，现已加机制护栏）**：`apply_project_sentence_reassignments` → `_invalidate_overlapping_voiceprint_samples` 会按 `project_id` + 原 speaker + 时间重叠把全局声纹库样本**标记为 `invalidated`（软删）**——行/clip/embedding 全保留、退出匹配池、可用 `update_voiceprint_sample_status` 恢复,不再物理 DELETE。project_id 是内容寻址的,拷贝项目 + `--apply` 指向真实库时仍会把真实样本踢出匹配池(可恢复但会立刻影响匹配结果),验证时依旧用 `resplit --apply --store-dir <拷贝>` 隔离。dry-run(不加 `--apply`)只读不动库,安全。
...
- **dry-run「零写盘」靠 `analyze_project_resplit(read_only=True)` 兑现，别把它当默认行为删掉**：嵌入本身要落 probe clip（embed 必须读音频文件）+ persist 共享 clip 缓存 `embeddings/clip_embeddings.json`（probe/cluster/sample 三方共用,存 raw 向量,见 `speaker_clip_embeddings.py`；0.21.0 之前在 `tmp/voiceprint_clips/` 下，见「Project Layout Notes」），默认会写进项目目录。read_only 路径把**暖缓存仍从真实项目读**进内存复用，但把 clip 抽取 + cache persist 整体重定向到一次性 `TemporaryDirectory`（`_ClusterContext.project_root` 换成 scratch，`source` 仍指真实音频；clip 持有的是向量不是文件路径，嵌完即弃）。CLI dry-run 传 `read_only=True`，`project run` / `--apply` 不传（那两条**该**写 cache 暖后续嵌入）。谁要把 read_only 去掉或让 dry-run 也 persist，就破坏了预览契约（实测参考项目 dry-run 后 635 个 tmp/speakers/exports 文件 mtime 零变化）。
- **promotion 必须同时赢过本 track 的留一质心（`track_lead ≥ promote_track_lead_margin`，默认 0.05），别只比库向量。** 已指派人的库向量是跨场次锚点，可能很弱（3 样本 / 1 场 / 25s 的条目在库里占一半以上），同一个人的子群对它只有 ~0.5、对某个同条件录的薄库条目却有 0.66，从库的角度就是「混进来一个人」；跟本 track 其余句子的质心比却有 0.91+。2026-09-22 两场家庭录音因此各自被塞进 2~3 个不可能在场的供应商，事后 stabilization 还把更多句子搬进假簇放大错误。回归全部历史 promotion：假新建对本 track 的领先都 ≤ −0.20，真新建都 ≥ +0.11，中间是宽空档。未指派 track 从一开始就用自身质心做参照（所以从没出过这种假阳性），改动只是把已指派 track 拉齐。谁要把这个闸门去掉或把参照换回库向量，就是重开这个事故。
- **diarization 原始 track 数 ≥ manifest `asr.speaker_count_hint` 时 `project run` / `speakers rerun` 跳过 resplit（`resplit_skip_reason`）。** 它是 under-split 救援，簇数够了按定义没得救，跑一遍只是把每句非低信息句嵌一遍（173 分钟项目 2h16m）然后误拆。**track 数必须从 `asr/raw_result.json` 数（`postprocess.raw_speaker_ids`），不能数 `sentences.json`**：归一化会丢掉纯填充词的 track（少数），前一轮 resplit / stabilization 又可能往里 mint 过新 id（多数），两个方向都会让闸门判错；raw 缺失才退回归一化转写。`speakers resplit --apply` 是用户明确要求，不受闸门约束；没传 count 的项目行为不变。
- **本地声纹模型推理默认 1 个 torch 线程（`voiceprint.torch_threads`），并发靠线程池。** 实测同一条 8.9s clip：8 线程 5~7s、2 线程 0.18s、1 线程 0.25s——共享的 8 核机器上 OpenMP 吃满全核互相 spin-wait，慢 25~30 倍；3,087 次嵌入的稳定化因此跑了 3h21m（应约 18 分钟）。任何新的 torch 推理路径都要在模型加载处调 `configure_torch_threads()`；性能基准先看 `torch.get_num_threads()`。
...
- **`projects_dir / project_id` 不是项目目录，永远别这么拼。** 项目 id 是内容寻址的，但**目录名不是** —— `--project-dir` 建的项目保留用户给的目录名，id 只活在 manifest 里。所有 ref 解析（`resolve_project_ref` → `list_projects`）都是**扫 manifest 按 id 匹配**，所以拼路径会把一个 review 打得开的项目判成「不存在」。要 id→目录，走 `list_projects(projects_dir).projects` 建映射。（这个 bug 原本潜伏在 overlap 检查里且是**静默**的——只是跳过检查；一旦这个判据被拿去渲染「来源已删除」徽章和体检告警，就变成可见的误报，还会催用户去补采一个根本没问题的人。）
- **「样本的来源项目还在不在」判据必须和 review 页自己那条链一致，否则两种方向的错都会犯。** review 走 `_load_review_transcript_result`：**`sentences_corrected.json` 优先**、`include_low_information=True`、且**至少要有一条带 speaker 的非空句**（否则 `RuntimeError("No detected speakers found")`）。只读 raw `asr/sentences.json` 会同时「把能打开的说成已删除」（raw 没了但 corrected 在）和「把打不开的说成可用」（corrected 是坏的 / 全是无 speaker 空句）。overlap 检查也该吃同一份转写：判据是「clip 旁边**谁**在说话」，而 review 里做过的重指派才是更准的答案；低信息句尤其**不能过滤**——邻座那句「嗯嗯对」被转写过滤掉了，但它实实在在录进了 clip，滤掉就等于把这个检查专门要抓的串场藏起来。
...
- **采样片段不能把旁边那个人一起采进来（padding 必须 gap-aware）。** clip 窗口原本是 `[句首-padding, 句尾+padding]`，而密集对话里邻座间隔常常是 **0.00s**——实测一通两人电话里，0.5s 的 padding 让每条 clip 含 **10~13%** 的另一个人的声音。`_padding_limits` 现在遇到别人说话就停。**`_boundary_score` 其实早就检测到了**（gap<500ms 给 0.45），但它权重只有 0.20、最多扣 0.11 分，压不住一段又长又完整的污染片段；更讽刺的是那个「<500ms」档位恰好就是 padding=0.5s 保证会污染的区间。所以重叠风险升格为**独立布尔量** `has_overlap_risk()` 而不是分数里的一项，让调用方能直接拒绝。
- **「默认推荐」= 推荐，所以绝不预选重叠片段**，但要**仍然列出且可选**——薄来源上，一个用户知情选择的次品样本胜过没有样本。只有在没有更干净的片段时才放回默认选择。
- **污染的危害是有方向的，不是随机噪声**：它把这个人的质心往「跟他对话的那个人」拉，而那恰恰是最难区分的一对。实测把 3 条这样的样本并入后，两人质心 cos 从 0.225 升到 0.323。**去掉 padding 只能救回一部分**（0.323→0.311），剩下的是片段本身取自交替密集处——所以标记必须独立于 padding 修复而存在。
- **库体检的 `overlapped-samples` 需要 `projects_dir`，不传就不做这项检查。** 这类样本一致性和可用性检查都发现不了（有向量、已启用、还和同一批同样被污染的样本彼此自洽），唯一能看出来的地方是来源项目的转写。**「没查」绝不能渲染成「查过且干净」**，所以缺 projects_dir 时返回空而不是猜。
- **报「N 条有问题」而不指出是哪 N 条，等于没报。** 体检算的是 per-person 计数，但用户点「查看样本」落到声纹库后，每一行长得都一样——问题被原样丢了回去。判定因此下沉到 `voiceprint_sample_overlap.overlapped_sample_ids()`（health 复用它做计数，规则单一来源），样本列表接口按行给 `overlap_risk`，深链带 `&filter=overlap` 直接落到那几行。**接口返回 `bool | None`，`None` = 来源项目读不到、没查过**：UI 上「没查」和「干净」都渲染成无告警，所以在数据层就不能共用一个值。新增任何「某人有 N 个 X」的体检项时，先问「点进去看得见是哪 N 个吗」。
- **「排除这几条」不能走批量质量接口。** `POST /people/{ref}/quality/exclude` 按一致性 label 过滤，而被污染的样本**分数通常正常**（实测三条 0.78/0.84/0.85，理由都是 `cluster-consistent`），传进去会报成功但一条不动——又一个「按钮必须是能成功的」的反例。改为逐条 PATCH status。
- **受影响样本 == 全部可匹配样本时，不给排除按钮。** 排空匹配池（人直接消失出匹配）比质心偏移更糟，那时唯一正确的第一步是先补采。这条分支实测库里跑不到，验证用 `page.route` 改写响应造出来。
- **调 `plan_capture` 必须显式传 `store_dir`。** 它要拿库解析每个说话人的 person，默认下来就会在 `--store-dir` 隔离副本里静默认到真实库的人。
- **采集是「必答题」不是「报告」**：跑完会留一个待决事务，不 accept/rollback 会让之后所有写库操作 409（直到 6h 清扫）。所以 `CaptureResultModal` 是 `web/src/components/` 下的共享组件而非各页一份，且每个能发起采集的入口都要带 `beforeunload`/`pagehide` 兜底回滚。新增采集入口时别只抄按钮忘了抄这套。**整个补采面板同理已抽成 `web/src/components/SampleSourcePanel.tsx`**（库质量页与声纹库页共用）：这套流程多一份实现就是多一次忘掉回滚的机会。
