...
- A second local provider `local-campp` (3D-Speaker CAM++ zh-cn, vendored in `src/app/infra/campplus.py`) coexists per model key; the default is switched via the `voiceprint.provider` config key, not code. Do not add ModelScope SDK or speakerlab as dependencies — neither supports this project's Python floor; the vendored torch-only definition plus a sha256-pinned checkpoint download is deliberate.
