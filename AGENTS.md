...
- **转写 track 数 ≥ manifest `asr.speaker_count_hint` 时 `project run` / `speakers rerun` 跳过 resplit（`resplit_skip_reason`）。** 它是 under-split 救援，簇数够了按定义没得救，跑一遍只是把每句非低信息句嵌一遍（173 分钟项目 2h16m）然后误拆。`speakers resplit --apply` 是用户明确要求，不受闸门约束；没传 count 的项目行为不变。
