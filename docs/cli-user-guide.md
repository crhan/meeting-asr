...
meeting-asr project speakers apply --map 0=Name   # advanced/scripted
...
这时每个 speaker 都会是 `status=no-candidate`，表示当前没有可匹配的人。

`inspect` 是只读诊断命令。它会同时显示 speaker 样例和声纹建议，但不会写入姓名：
...
- 顶部 `Auto` 是自动匹配质量：matched/below-threshold/no-candidate 数量、平均分、最高分。
...
`apply --map` 是 advanced/scripted 入口，适合脚本或已经确认好的映射，不是人类默认修正入口：

```bash
meeting-asr project speakers apply --map 0=Name
```

人类确认 speaker 姓名时，优先使用 `meeting-asr project review PROJECT`。
旧的 prompt mode 仍保留给专家排障：`meeting-asr project speakers apply PROJECT`。
在这个模式里，`/audio` 播放当前终端显示的样例批次，`/more` 输出更多样例；如果最终仍不知道是谁，直接回车保留匿名 label。
